...
class ProposalExporter:
    ...
    def export_pdf(
        ...
        # Header section
        story.append(Paragraph("NovaIntel AI", company_style))
        ...
        details_data = [
            ...
            ("Document ID:", f"RFP-{datetime.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"),
        ]
        ...
        Generated by NovaIntel AI | {current_date}<br/>
    ...
    def export_docx(
        ...
        run = company_para.add_run('NovaIntel AI')
        run.bold = True
        run.font.size = Pt(20)
        ...
        current_date = datetime.now().strftime('%B %d, %Y')
        doc_id = f"RFP-{datetime.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
        ...
        footer_run = footer_para.add_run(
            f'Generated by NovaIntel AI | {current_date}\n'
            'This is an AI-generated proposal based on your RFP analysis'
        )
